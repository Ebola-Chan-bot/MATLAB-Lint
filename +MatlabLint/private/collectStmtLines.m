...
%COLLECTSTMTLINES 收集函数内所有语句行：赋值、引用、控制块头尾、END。
% extraLines: 额外的行号数组（如函数结束行）
...
% mtree 不产出 END 节点，从控制块头推断 end 行
...
for ki = 1:numel(blockKinds)
    ...
    for i = 1:numel(ix)
        ...
        [el, ~] = pos2lc(nd, righttreepos(nd));
        if el >= fStart && el <= fEnd
            stmtLines(end + 1) = el; %#ok<AGROW>
        end
    end
end
